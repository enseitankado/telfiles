...
- **Отдельные таблицы для файлов, ссылок и каналов** — сортировка + фильтры по столбцам, сужение по каналу / типу / размеру / дате; вкладка «Каналы» показывает количество участников, файлов и поддерживает массовые операции.
- **Индекс содержимого торрентов** — файлы `.torrent` автоматически разбираются; их внутренние пути добавляются в базу данных и включаются в полнотекстовый поиск на вкладке «Файлы».
- **Загрузки и передача** — скачанные файлы отображаются во вкладке истории. Сохранённые файлы можно автоматически копировать или перемещать по FTP, SFTP или в локальную директорию (NAS / внешний диск). **Планирование полосы пропускания** позволяет загружать большие файлы только в указанные часы.
