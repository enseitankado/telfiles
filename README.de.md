...
- **Separate Grids für Dateien, Links & Kanäle** — Sortierung + Filter pro Spalte, Eingrenzung nach Kanal / Typ / Größe / Datum; der Kanäle-Tab zeigt Mitgliederzahlen, Dateianzahlen, unterstützt Massenoperationen und Shift-Klick-Bereichsauswahl bei Checkboxen.
- **Torrent-Inhaltsindex** — `.torrent`-Dateien werden automatisch geparst; die enthaltenen Dateipfade werden zur Datenbank hinzugefügt und in die Volltextsuche im Dateien-Tab aufgenommen. Große Archive werden auf 1.000 Einträge begrenzt, die verbleibende Anzahl wird angezeigt.
- **Semantische Suche** — optionale pgvector-Integration; Dateinamen werden mit einem lokalen Modell eingebettet, sodass Ergebnisse nach Bedeutung statt nur nach Schlüsselwort-Übereinstimmung sortiert werden können.
- **Downloads & Transfer** — heruntergeladene Dateien werden im Verlaufs-Tab mit lokalem Pfad-Tooltip verfolgt; alle lokalen Dateien können per Klick gelöscht werden. Gespeicherte Dateien können automatisch per FTP, SFTP oder in ein lokales Verzeichnis (NAS / externe Festplatte) kopiert oder verschoben werden. **Bandbreitenplanung** ermöglicht Downloads großer Dateien nur in festgelegten Zeitfenstern. Ziel-Einstellungen werden sitzungsübergreifend gespeichert.
...
- **In-App-Aktualisierungsbenachrichtigungen** — beim Start prüft die App den neuesten Commit auf GitHub und zeigt ein Banner mit einem Ein-Klick-Kopierbefehl an, wenn eine neue Version verfügbar ist.
- **Sichere Schema-Migrationen** — eine versionierte `schema_migrations`-Tabelle verfolgt jede angewendete Änderung; breaking changes (Typänderungen, Umbenennungen, Löschungen) laufen in eigenen Transaktionen; bei einem Fehler verweigert die App den Start, sodass Daten nie in einem unvollständigen Zustand verbleiben.
